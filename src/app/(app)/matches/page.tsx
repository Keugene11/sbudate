  ...
  return (
    <div className="max-w-lg mx-auto bg-white min-h-screen">
      <div className="px-5 pt-4 pb-3">
        <h1 className="text-[24px] font-semibold text-gray-900">Matches</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center pt-16 px-8 text-center animate-slide-up">
          ...
          <p className="text-gray-400 text-[14px] leading-relaxed">When you and someone like each other, you can chat here.</p>
        </div>
      ) : (
        <div className="stagger">
          {matches.map((match) => (
            <Link key={match.match_id} href={`/chat/${match.match_id}`}
              className="flex items-center gap-3 px-5 py-3 press active:bg-gray-50 transition-colors duration-150">
              ...
              <div className="flex-1 min-w-0 py-1">
                ...
                <p className={`text-[14px] truncate mt-0.5 ${match.unread ? "text-gray-900" : "text-gray-400"}`}>
                  {match.last_message || "Say hello 👋"}
                </p>
              </div>
              {match.unread && <div className="w-2 h-2 rounded-full bg-accent flex-shrink-0" />}
            </Link>
          ))}
        </div>
      )}
