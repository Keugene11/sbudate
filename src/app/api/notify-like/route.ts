...
export async function POST(request: Request) {
  ...
  const { data: like } = await admin.from("likes").select("id, from_profile_id, to_profile_id, comment").eq("id", likeId).single();
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `Like comment: ${sender?.first_name} → ${recipient?.first_name}`,
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 16px; background: #fff;">
          <p style="font-size: 13px; color: #999; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 600; margin: 0 0 20px;">Like Comment Review</p>
          <p style="font-size: 14px; color: #666; margin: 0 0 12px;">
            <strong style="color: #111;">${sender?.first_name}</strong> liked <strong style="color: #111;">${recipient?.first_name}</strong>'s profile with a comment:
          </p>
