...
export async function GET(request: Request) {
  ...
  if (!likeId || !action || !["approve", "reject"].includes(action)) {
  ...
  if (action === "reject") {
    // Remove the comment but keep the like
    await admin.from("likes").update({ status, comment: null }).eq("id", likeId);
  } else {
    await admin.from("likes").update({ status }).eq("id", likeId);
  }
  ...
  return new NextResponse(`
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #fafafa;">
      <div style="text-align: center; padding: 32px;">
        ...
        <h1 style="font-size: 22px; font-weight: 700; color: #111; margin: 0 0 8px;">Comment ${action === "approve" ? "approved" : "rejected"}</h1>
        <p style="font-size: 15px; color: #666; margin: 0;">${action === "approve" ? "The comment is now visible." : "The comment has been removed."}</p>
      </div>
    </body>
    </html>
