...
export async function GET(request: Request) {
  ...
  if (error) {
    return new NextResponse("<h1>Error updating profile</h1>", { status: 500, headers: { "Content-Type": "text/html" } });
  }
  ...
  return new NextResponse(`
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #fafafa;">
      <div style="text-align: center; padding: 32px;">
        ...
        <h1 style="font-size: 22px; font-weight: 700; color: #111; margin: 0 0 8px;">Profile ${label}</h1>
        <p style="font-size: 15px; color: #666; margin: 0;">The user has been notified.</p>
      </div>
    </body>
    </html>
